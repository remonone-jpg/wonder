 ...
 * 동적으로 그린다.
 *
 * 은하의 글이 사는 곳도 여기다. 오른쪽 읽기 패널의 설명(`easy`/`detail`)과
 * 3D 무대 1단계의 글이 같은 문장이라, 두 곳에 나눠 적으면 한쪽만 고치는
 * 날이 온다. `data/cosmos/<은하>.ts` 는 이름·색점·수치·찾아보기만 들고
 * 설명은 여기서 가져간다.
 */
 ...
 * 은하 하나를 세운다.
 *
 * 1단계의 글을 `views` 에 따로 적지 않고 여기서 옮겨 붙이는 이유는, 그것이
 * 이 항목의 대표 설명(`easy`/`detail`/`observe`)과 같은 문장이기 때문이다.
 * 두 벌로 적으면 한쪽만 고치게 된다. 1단계의 **제목과 부제**는 은하마다
 * 다르므로 `views.wide` 로 덮어쓴다.
 */
function galaxy(base: Omit<GalaxyModel, "views">, views: ViewOverrides = {}): GalaxyModel {
  return {
    ...
    views: withViews({
      ...views,
      wide: { easy: base.easy, detail: base.detail, observe: base.observe, ...views.wide },
    }),
  };
}
...
export const galaxyModels: readonly GalaxyModel[] = [
  galaxy({
    ...
    wide: { name: "안에서 그린 지도", note: "우리의 주소가 들어 있는 은하를 한눈에 봐요" },
  ...
  galaxy({
    ...
    wide: { name: "가장 가까운 큰 은하", note: "250만 년 전의 이웃을 지금의 하늘에서 봐요" },
  ...
  galaxy({
    ...
    wide: { name: "남쪽 하늘의 작은 이웃", note: "작은 이웃 은하가 남쪽 하늘에 남긴 얼룩을 봐요" },
  ...
  galaxy({
    ...
    wide: { name: "팔이 없는 거대 은하", note: "둥근 별 구름과 한 줄기 제트를 함께 봐요" },
    ...
    light: { name: "제트의 흔적", note: "전파와 X선에서 더 강하게 드러나는 제트를 생각해 봐요", easy: "별빛을 줄이면 중심에서 뻗는 제트가 잘 보여요.", detail: "M87에서는 오래된 별빛을 줄이고 제트를 강조합니다. 제트는 사건지평선 안에서 탈출한 물질이 아니라, 블랙홀 주변의 물질과 자기장에 관계된 흐름입니다. 실제 제트는 전파·가시광선·X선으로 연구하며, 이 푸른 선은 그 방향을 단순화한 도해입니다. 관측에서 한쪽이 특히 밝은 데에는 우리 쪽으로 움직이는 물질의 상대론적 밝기 증가도 관여합니다.", observe: "둥근 별 무리가 흐려지고 제트가 남아요." },
  }),
];
